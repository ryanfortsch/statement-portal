...
--   shaped "<rule>:<booking_id>" (today: "trashbags:<booking_id>"). One slip
--   per rule per stay, ever — mirrors from_guest_request_key semantics, so a
--   dismissed slip stays dismissed and the cron never re-files it. A separate
--   column (rather than reusing from_guest_request_key) keeps the origin
--   filterable: guest-request means "the guest asked", prep-rule means "the
--   calendar implies it".
