...
export default async function ProjectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--paper)', color: 'var(--ink)' }}>
      ...
      <HelmBreadcrumb
        trail={[
          { label: 'Prospects', href: '/properties/prospects' },
        ...
        {(ownerPortfolio.properties.length > 0 || ownerPortfolio.prospects.length > 0) && (
          ...
          >
            ...
            {ownerPortfolio.prospects.map((op) => (
              <Link key={op.id} href={`/prospects/${op.id}`} style={{ color: 'var(--ink-3)', textDecoration: 'underline', textUnderlineOffset: 3 }}>
...
function DeliverableActions({
  ...
  // render/guide/contract intentionally stay at /projections: they're public
  // PDF surfaces with external callers (proxy allowlist + Puppeteer). Only
  // readiness moved with the workroom to /prospects.
  const openHref =
    openSlug === 'readiness'
      ? `/prospects/${projectionId}/readiness`
      : `/projections/${projectionId}/${openSlug}`;
  return (
