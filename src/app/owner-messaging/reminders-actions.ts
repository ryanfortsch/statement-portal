...
/**
 * Server actions behind the owner ProactiveRemindersPanel. This file
 * hardcodes the audience ('owner') and its own revalidate path, so the
...
const AUDIENCE = 'owner';
const PAGE = '/owner-messaging';
...
export async function polishProactiveForAction(
  ...
  if (!res.ok) return { ok: false, error: explainError(res.error) };
  // english is '' for owners by contract; pass through defensively anyway.
