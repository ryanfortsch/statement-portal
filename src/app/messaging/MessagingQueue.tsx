...
export function MessagingQueue({ initialPending }: Props) {
  ...
  // Refresh inside a transition. A bare router.refresh() re-suspends the queue's
  // Suspense boundary (listApprovals is an uncached network call), which briefly
  // swaps in the QueueSkeleton fallback and UNMOUNTS everything below it --
  // including the "New proactive message" form, erasing whatever the operator
  // was mid-typing (reported 2026-07-01). A transition makes React keep the
  // current UI mounted while the new payload streams, so client state survives.
  ...
  // last 15 seconds shows up without a manual reload. Hidden tabs skip the
  // tick entirely: each one is a full server render of this page, and a pile
  // of open tabs refreshing in wall-clock lockstep is exactly the burst that
  // gets requests shed on Hobby and trips the error boundary. The
  // visibilitychange listener catches a tab up the moment it's fronted, and
  // the jittered period (picked once per mount) keeps tabs drifted apart.
  useEffect(() => {
    const t = setInterval(() => {
      if (!document.hidden) softRefresh();
    }, Math.round(REFRESH_MS * (0.8 + Math.random() * 0.4)));
    const onVisible = () => {
