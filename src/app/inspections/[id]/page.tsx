...
async function getInspection(id: string): Promise<{
  ...
  const [{ data: property }, { data: items }, { data: results }, { data: zoneRows }] =
    await Promise.all([
      supabase
        .from('properties')
        .select('id, name, title, city, bedrooms, pullout_linens_location')
...
export default async function InspectionInProgressPage({
  ...
  const [initialNotes, { data: workSlipsData }] = await Promise.all([
    loadStepperNotes(supabase, { inspectionId: id, propertyId: property.id }),
    supabase
      .from('work_slips')
  ...
  return (
    <Stepper
      ...
      pulloutLinensLocation={property.pullout_linens_location}
      cards={cards}
