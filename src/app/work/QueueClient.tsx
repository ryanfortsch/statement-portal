...
export function QueueClient({ workSlips, snoozedSlips, tasks, properties, myEmail, slipCommentCounts, taskCommentCounts, reporterNames }: Props) {
  ...
  return (
    <>
      {/* The whole head of the page is one row. The two board faces are the
          section heading, so picking a tab and naming the list are the same
          gesture, and the + buttons ride along instead of hanging off a
          title bar of their own. The filter pills sit below, scoped to
          whichever face is up — only the ones with > 0 matches render, plus
          the selected one (so a pill can't vanish out from under your finger
          when its count hits zero), plus the All reset. Snoozed is the
          exception: it's the only door to a separate server-side bucket, so
          on Property Work it always renders, dimmed at zero. */}
      <section className="max-w-[1100px] mx-auto px-10" style={{ paddingTop: 36, paddingBottom: 24, width: '100%' }}>
        ...
        <div className="flex items-center justify-between flex-wrap" style={{ gap: 12 }}>
          ...
          {/* Quiet lens link: the guest-gear matrix is its own page so the
              board stays the board. Rode in on a shelf of its own until the
              title block above it went away. */}
