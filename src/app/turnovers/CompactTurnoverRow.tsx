        ...
        <div className="rt-tn-exp" onClick={(e) => e.stopPropagation()}>
          ...
          {!isDone && (
            <div className="rt-tn-affordances">
              ...
              {t.fieldPacket && (
                <Link href={`/fieldwork/packets/${t.fieldPacket.packetId}`} style={{ ...chipLink, color: fieldChipColor(t.fieldPacket.status), fontWeight: 600 }}>
...
function FieldCredit({ fp }: { fp: NonNullable<Turnover['fieldPacket']> }) {
  ...
  return (
    <Link href={`/fieldwork/packets/${fp.packetId}`} className="rt-tn-field" title={title}>
