...
// This static /guests/contacts segment must win over the /guests/[id] sibling.
// Next resolves static segments before dynamic ones, and contact ids are
// uuids (audience_contacts.id), so no real contact can collide with the
// literal path 'contacts'.
      ...
      <section className="max-w-[1100px] mx-auto px-10" style={{ width: '100%', paddingBottom: 24 }}>
        <form method="get" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(search || tag || status !== 'all') && (
            <Link href="/guests/contacts" style={{ fontSize: 12, color: 'var(--ink-3)', textDecoration: 'underline' }}>
              Clear
            </Link>
          )}
        </form>
        ...
        {stats.topTags.length > 0 && (
          <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {stats.topTags.map((t) => {
              ...
              const href = qs ? `/guests/contacts?${qs}` : '/guests/contacts';
              return (
                <Link
                  key={t.tag}
                  href={href}
