      ...
      <section className="max-w-[900px] mx-auto px-10" style={{ width: '100%', paddingTop: 28, paddingBottom: 48 }}>
        ...
        <div style={{ border: '1px solid var(--rule)', borderRadius: 10, padding: '16px 20px', marginBottom: 28, background: 'var(--paper-2, #fff)' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: 'var(--ink-3)', lineHeight: 1.7 }}>
            ...
            <li>Back here as staff: on the <Link href="/fieldwork/packets" style={{ color: 'var(--tide-deep)' }}>packets board</Link>, <strong>Approve</strong> each submitted packet, then <strong>Mark paid</strong>.</li>
            <li>Try the operator side yourself too: bundle from the calendar or the <Link href="/fieldwork/packets/maintenance" style={{ color: 'var(--tide-deep)' }}>Maintenance jobs</Link> page.</li>
        ...
        {state.seeded && (
          <p style={{ fontSize: 12, color: 'var(--ink-4)' }}>
            ...
            <Link href="/fieldwork/packets/maintenance" style={{ color: 'var(--tide-deep)' }}>Maintenance jobs</Link> page.
          </p>
        )}
...
function PacketRow({ p }: { p: FieldTestPacket }) {
  return (
    <div style={{ border: '1px solid var(--rule)', borderRadius: 10, padding: '12px 16px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', background: 'var(--paper-2, #fff)' }}>
      ...
      <Link href={`/fieldwork/packets/${p.id}`} style={{ fontSize: 12, color: 'var(--tide-deep)', textDecoration: 'none' }}>Operator view →</Link>
