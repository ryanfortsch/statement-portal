...
/**
 * Server actions behind the contractor ProactiveRemindersPanel. This file
 * hardcodes the audience ('contractor') and its own revalidate path, so the
 * shared client panel just receives these via its `actions` prop and never
 * needs to know which page it lives on. Mirrors the cleaner
 * src/app/cleaner-messaging/reminders-actions.ts split: the two fetches
 * degrade failures to empty arrays so the panel renders instead of erroring.
 */

const AUDIENCE = 'contractor';
const PAGE = '/contractor-messaging';
