 ...
 * the higher rate for that property. Charges created before that date
 * were collected at the 11.7% base rate, so the stripe-sync collected-
 * net rebuild must invert them at 11.7% -- flipping the multiplier
 * retroactively would silently rewrite already-recognized rent on old
 * statements. That's why the lookup takes the charge-creation date, not
 * just the property.
 *
