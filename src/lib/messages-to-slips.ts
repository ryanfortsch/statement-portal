...
File an issue when work remains: a repair, a replacement, a restock, or a follow-up verification. A host reply that ACKNOWLEDGES or PROMISES to handle a problem ("we'll get it sorted", "we'll take a look after checkout", "someone will swing by") does NOT resolve it -- that promise is exactly what a work slip exists to track. Treat an issue as resolved only when the thread shows the fix was completed AND the guest confirmed it works with nothing left to verify.

Do NOT file: informational questions the host answered (directions, where amenities live, how something works), requests fulfilled with no underlying defect (late checkout, early check-in, discounts, extension offers, parking asks), pure gratitude, a guest apologizing for something they did, or ideas for new signage/process. A message can mix both kinds: a parking ask alongside a broken-lock report still yields the broken-lock issue. A one-time courtesy delivery is not an issue by itself, BUT if it reveals the house's own stock was missing or depleted (e.g. one beach towel in the whole house), file an inventory verification for the turnover.
